...
class Joint:
    ...
    def make_control_xml(self):
        """
        Generate the control_xml and hold it by self.control_xml
        ...
        cntrl = Element('ros2_control')
        cntrl.attrib = {'name':'GazeboSystem'}
        cntrl.attrib = {'type':'system'}
        
        # hardware_type = SubElement(cntrl, 'hardware')
        # hardware_plugin= SubElement(hardware_type,'plugin')
        # hardware_plugin.text='gazebo_ros2_control/GazeboSystem'
        
        joint = SubElement(cntrl, 'joint')
        joint.attrib = {'name':self.name}
        pos_command_interface = SubElement(joint, 'command_interface')
        pos_command_interface.attrib= {'name':'position'}
        pos_command_interface_min_param=SubElement(pos_command_interface,'param')
        pos_command_interface_min_param.attrib={'name':'min'}
        pos_command_interface_min_param.text=str(self.lower_limit)
        
        pos_command_interface_max_param=SubElement(pos_command_interface,'param')
        pos_command_interface_max_param.attrib={'name':'max'}
        pos_command_interface_max_param.text=str(self.upper_limit)
        
        vel_command_interface = SubElement(joint, 'command_interface')
        vel_command_interface.attrib= {'name':'velocity'}
        vel_command_interface_min_param=SubElement(vel_command_interface,'param')
        vel_command_interface_min_param.attrib={'name':'min'}
        vel_command_interface_min_param.text='10'
        
        vel_command_interface_max_param=SubElement(vel_command_interface,'param')
        vel_command_interface_max_param.attrib={'name':'max'}
        vel_command_interface_max_param.text='100'
        
        pos_state_interface=SubElement(joint,'state_interface')
        pos_state_interface.attrib={'name':'position'}
        pos_state_interface_min_param=SubElement(pos_state_interface,'param')
        pos_state_interface_min_param.attrib={'name':'initial_value'}
        pos_state_interface_min_param.text='0.0'

        vel_state_interface=SubElement(joint,'state_interface')
        vel_state_interface.attrib={'name':'velocity'}

        eff_state_interface=SubElement(joint,'state_interface')
        eff_state_interface.attrib={'name':'effort'}
        vel_state_interface_min_param=SubElement(vel_state_interface,'param')
        vel_state_interface_min_param.attrib={'name':'initial_value'}
        vel_state_interface_min_param.text='0.0'
